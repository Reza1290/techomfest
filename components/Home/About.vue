<template>
  <div
    class="h-[1024px] flex items-center justify-between center py-[147px] px-[101px]"
  >
    <div class="w-[58%] mr-3">
      <h3 class="font-bold text-7xl text-primary">Apa itu LokaBestari?</h3>
      <!-- <p class="mt-4 text-2xl italic font-normal text-accent">
        <span class="font-medium">loka</span> /lo·ka/ = dunia; tempat
      </p> -->
      <p class="mt-4 text-2xl italic font-normal text-accent">
        <span class="font-medium">bestari</span> /bes·ta·ri/ = luas dan dalam
        pengetahuannya; berpendidikan baik; baik budi pekerti
      </p>
      <p class="text-2xl font-normal text-black mt-7">
        LokaBestari merupakan situs web yang mempersembahkan keanekaragaman
        budaya yang ada di Indonesia. <br />
        <br />Di LokaBestari, kami membangun sebuah ruang digital yang
        mempersembahkan kekayaan tradisi, seni, dan budaya dari penjuru
        nusantara. Dari kesenian, hidangan otentik, dan masih banyak yang
        lainnya. <br />
        <br />LokaBestari memberi kemudahan bagi Anda untuk mengeksplor dan
        mengedukasi Anda tentang warisan budaya yang ada di Indonesia.
      </p>
    </div>
    <div id="about" class="box-border flex w-[30%] justify-end py-[40px]">
      <div class="flex flex-wrap justify end">
        <img
          src="~/assets/img/home/about1.png"
          class="max-w-[515px] max-h-[610px] rounded-3xl z-10 hover:scale-105 transition-all duration-500 hover:shadow-2xl"
          alt=""
        />
        <img
          src="~/assets/img/home/about2.png"
          class="w-[473px] h-[329px] -mt-[55%] -ml-[15%] z-20 rounded-3xl outline outline-8 outline-[#EFFFF3] hover:outline-none hover:scale-105 transition-all duration-500 hover:shadow-2xl"
          alt=""
        />
      </div>
    </div>
  </div>
</template>
